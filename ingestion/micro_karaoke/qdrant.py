"""Upload normalized micro-karaoke points with deterministic IDs."""
...
CANONICAL_TYPES: dict[str, type | tuple[type, ...]] = {
    ...
    "microphone_type": str,
    "wireless_band": str,
    "rf_frequency_min_mhz": (int, float),
    "rf_frequency_max_mhz": (int, float),
    "audio_frequency_min_hz": (int, float),
    "audio_frequency_max_hz": (int, float),
    "distortion_pct": (int, float),
...
def validate_metadata(metadata: Any, index: int) -> dict[str, Any]:
    ...
    if metadata["category_scope"] != "karaoke_microphone":
...
def main() -> None:
    ...
    if not isinstance(products, list) or not products:
        raise ValueError("Không có dữ liệu micro karaoke đã chuẩn hóa để upload")
    client = QdrantClient(
        url=url,
        api_key=api_key,
