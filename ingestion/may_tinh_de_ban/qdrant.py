"""Upsert normalized desktop points using Qdrant Cloud Inference."""
...
def load_products() -> list[dict]:
    ...
    if not isinstance(products, list) or not products:
        raise ValueError("Không có dữ liệu desktop đã chuẩn hóa để upload")
    return products


def point_generator(products: list[dict]) -> Iterator[models.PointStruct]:
    """Yield stable points and let Qdrant Cloud embed their semantic passage."""
    ...
    for index, product in enumerate(products):
        ...
        if not isinstance(metadata, dict) or metadata.get("category_scope") != "desktop":
            raise ValueError(
                f"Sản phẩm index {index} chưa có metadata.category_scope=desktop"
            )
        seen_ids.add(product_id)
        yield models.PointStruct(
            id=str(uuid.uuid5(POINT_NAMESPACE, product_id)),
            vector=models.Document(
                text=f"passage: {text}", model=EMBEDDING_MODEL
            ),
            payload={
