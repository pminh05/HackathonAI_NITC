"""Upsert normalized monitor points using Qdrant Cloud Inference."""
...
COLLECTION_NAME = "manhinhmaytinh"
...
def load_products() -> list[dict]:
    ...
    if not isinstance(products, list) or not products:
        raise ValueError("Không có dữ liệu màn hình đã chuẩn hóa để upload")
    return products
...
def point_generator(products: list[dict]) -> Iterator[models.PointStruct]:
    ...
    for index, product in enumerate(products):
        ...
        if not isinstance(metadata, dict) or metadata.get("category_scope") != "monitor":
            raise ValueError(
                f"Sản phẩm index {index} chưa có metadata.category_scope=monitor"
            )
