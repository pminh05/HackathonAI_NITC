...
POST /chat
    ...
    │
    ├── Có memory ảnh hưởng quyết định ──> memory_confirmation_required
    │                                      │
    │                                      ▼
    │                         POST /chat/{thread_id}/resume
    │                           với kind=memory_confirmation
    │                                      │
    └── Còn thiếu thông tin ──> clarification_required
                                      │
                                      ▼
                         POST /chat/{thread_id}/resume
                              với kind=clarification
                                      │
                                      └── token* ──> completed
```
...
- Memory ảnh hưởng filter/ranking phải được xác nhận; tên gọi và style trả lời không cần xác nhận.
- Một memory question đã được quyết định sẽ không được hỏi lại trong cùng category/thread.

### Xác thực và ownership

- Không có header `Authorization` nghĩa là anonymous; luồng chat cũ vẫn hoạt động và không gọi Mem0.
- Người dùng đăng nhập gửi `Authorization: Bearer <supabase_access_token>` ở mọi request.
- Backend xác minh token với Supabase Auth và chỉ dùng UUID `sub` đã xác minh làm Mem0 `user_id`.
- Header có token sai/hết hạn trả `401`.
- Đọc, tiếp tục hoặc resume thread sai owner trả `404`, kể cả khi thread thật sự tồn tại.
- Các endpoint `/me/memories*` và `/chat/{thread_id}/memory-write` yêu cầu đăng nhập.
...
| `memory_confirmation_required` | Graph chờ xác nhận memory từ phiên trước | `thread_id`, `message`, `category`, `candidates` |
| `token` | Một phần nội dung trả lời của AI | `delta` |
| `completed` | Lượt xử lý đã hoàn tất | `thread_id`, `answer`, `selected_products`, `memory_write` |
...
Payload clarification mới (payload cũ chỉ có `answers` vẫn được chấp nhận):
...
Payload xác nhận memory:

```json
{
  "kind": "memory_confirmation",
  "decisions": [
    {"candidate_id": "mem-household", "action": "use"},
    {
      "candidate_id": "mem-preference",
      "action": "edit",
      "option_id": "energy_saving"
    },
    {"candidate_id": "mem-budget", "action": "ignore"}
  ]
}
```

`action` là `use`, `edit` hoặc `ignore`. `use` áp dụng đúng patch đã validate ở
server; `edit` phải gửi option thuộc catalog tương ứng (và `custom_answer` nếu là
`other`); `ignore` để clarification thường hỏi lại trường đó.
...
Giá trị `status` gồm `running`, `waiting_for_memory_confirmation`,
`waiting_for_clarification` và `completed`. Khi chờ memory, response có
`memory_confirmation` thay cho `questions`. Khi hoàn tất, `memory_write` có
`status` là `skipped`, `queued` hoặc `failed` và có thể có `event_id`.

### `GET /chat/{thread_id}/memory-write`

Proxy trạng thái Mem0 event của lượt đã hoàn tất, sau khi kiểm tra owner.
...
`status` là `skipped`, `pending`, `succeeded` hoặc `failed`. Khi `succeeded`,
`added_count=0` là kết quả hợp lệ và nghĩa là Mem0 không tìm thấy fact mới.

### `GET /me/memories`

Liệt kê memory đã sanitize của chính authenticated user. Hỗ trợ `page` (mặc định
1) và `page_size` (1–200, mặc định 50). Backend luôn thêm user filter, client
không được truyền `user_id`.
  ...
  "results": [
    {
      ...
      "memory": "Gia đình người dùng có 4 người.",
      "categories": ["household_context"],
      "metadata": {"active_category": "refrigerator"},
...
Kiểm tra memory thuộc authenticated user trước khi xóa. Memory không tồn tại hoặc
khác owner đều trả `404`.

### `DELETE /me/memories`

Xóa toàn bộ long-term memory đúng `user_id`. Frontend phải yêu cầu xác nhận trước
khi gọi. Thao tác không sửa ngược working profile trong thread đang chạy; hãy tạo
thread mới để chắc chắn không dùng dữ liệu đã recall trước đó.
